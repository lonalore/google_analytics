...
(function ($)
{
	...
	$(document).ready(function ()
	{
		...
		{
			console.group("Running Google Analytics for e107.");
			console.info(event);
				...
				if(e107.google_analytics.isInternal(this.href))
				{
					...
					else
					{
						if(e107.settings.google_analytics.trackDownload && e107.google_analytics.isDownload(this.href))
						{
							...
							console.info("Download url '%s' has been found. Tracked download as extension '%s'.", e107.google_analytics.getPageUrl(this.href), e107.google_analytics.getDownloadExtension(this.href).toUpperCase());
						...
						else
						{
							if(e107.google_analytics.isInternalSpecial(this.href))
							{
								...
								console.info("Click on internal special link '%s' has been tracked.", e107.google_analytics.getPageUrl(this.href));
		...
		$(document).bind("cbox_complete", function ()
		{
			...
			if(href)
			{
				console.info("Colorbox transition to url '%s' has been tracked.", e107.google_analytics.getPageUrl(href));
